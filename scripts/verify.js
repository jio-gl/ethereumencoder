/**
 * Script to verify contracts on any network
 * 
 * Usage:
 * npx hardhat run scripts/verify.js -- --contract <CONTRACT_NAME> --address <CONTRACT_ADDRESS> --network <NETWORK> [--args <CONSTRUCTOR_ARGS_FILE>]
 * 
 * Example:
 * npx hardhat run scripts/verify.js -- --contract ExampleToken --address 0x1234567890123456789012345678901234567890 --network localhost
 */
...
async function main() {
  ...
  const network = networkArg || "localhost";
  
  if (!contractName || !contractAddress) {
    console.error("Usage: npx hardhat run scripts/verify.js -- --contract <CONTRACT_NAME> --address <CONTRACT_ADDRESS> --network <NETWORK> [--args <CONSTRUCTOR_ARGS_FILE>]");
    process.exit(1);
  }
  ...
  try {
    ...
    try {
      ...
      console.log(`Successfully verified ${contractName} at ${contractAddress} on ${network}`);
    } catch (error) {
      // Check if the error is because the contract is already verified
      if (error.message.includes("Already Verified")) {
        console.log(`Contract ${contractName} at ${contractAddress} is already verified on ${network}`);
      } else {
